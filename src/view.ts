...
export class Graph3DView extends ItemView {
	...
	private fileContentCache = new Map<
		string,
		{ mtime: number; content: string; lowerCaseContent: string }
	>();
	private processedNodes = new Map<
		string,
		{ node: GraphNode; mtime: number }
	>();
		...
		if (!geometry) {
			...
			switch (shape) {
				...
				case NodeShape.Pyramid:
					geometry = new THREE.ConeGeometry(1 / 1.5, 1, perf ? 3 : 4);
					break;
				case NodeShape.Tetrahedron:
					geometry = new THREE.TetrahedronGeometry(1 / 1.2);
					break;
				default:
	...
	async onOpen() {
		...
		const viewWrapper = rootContainer.createEl("div", {
			cls: "graph-3d-view-wrapper",
		});

		this.graphContainer = viewWrapper.createEl("div", {
			cls: "graph-3d-container",
			attr: { tabindex: "0" },
		}); // Make it focusable
		this.messageEl = viewWrapper.createEl("div", {
			cls: "graph-3d-message",
		});
		this.counterEl = viewWrapper.createEl("div", {
			cls: "graph-3d-counter",
		});
	...
	private addLocalControls() {
		const controlsContainer = this.contentEl.createEl("div", {
			cls: "graph-3d-controls-container",
		});

		// Add Reset View button
		const resetViewButton = controlsContainer.createEl("div", {
			cls: "graph-3d-reset-toggle",
		});
		...
		this.settingsToggleButton = controlsContainer.createEl("div", {
			cls: "graph-3d-settings-toggle",
		});
		setIcon(this.settingsToggleButton, "settings");
		this.settingsToggleButton.setAttribute("aria-label", "Graph settings");
		this.settingsPanel = controlsContainer.createEl("div", {
			cls: "graph-3d-settings-panel",
		});
	...
	private renderSearchSettings(container: HTMLElement) {
		...
		new Setting(container).setName("Search term").addText((text) =>
			text.setValue(this.settings.searchQuery).onChange(
				debounce(
					async (value) => {
	...
	private renderAdvancedFilters(container: HTMLElement) {
		...
		new Setting(container).addButton((button) =>
			button.setButtonText("Add new filter").onClick(async () => {
	...
	private renderFilterSettings(container: HTMLElement) {
		...
		new Setting(container).setName("Show tags").addToggle((toggle) =>
			toggle.setValue(this.settings.showTags).onChange(async (value) => {
		...
		new Setting(container).setName("Show attachments").addToggle((toggle) =>
			toggle
		...
		new Setting(container).setName("Hide orphans").addToggle((toggle) =>
			toggle
	...
	private renderGroupSettings(container: HTMLElement) {
		...
		const render = () => {
			...
			this.settings.groups.forEach((group, index) => {
				new Setting(groupContainer)
					...
					.addColorPicker((color) =>
						color.setValue(group.color).onChange(async (value) => {
							group.color = value;
							await this.plugin.saveSettings();
							this.updateColors();
						}),
			...
			new Setting(groupContainer).addButton((button) =>
				button.setButtonText("Add new group").onClick(async () => {
	...
	private renderAppearanceSettings(container: HTMLElement) {
		...
		new Setting(container)
			...
			.setDesc(
				"Lowers geometry detail, disables link curvature and labels for better performance.",
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(container).setName("Node size").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
		...
		new Setting(container).setName("Tag node size").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.tagNodeSize)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container).setName("Attachment node size").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.attachmentNodeSize)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container).setName("Link thickness").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.linkThickness)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container).setName("Node shape").addDropdown((dd) =>
			dd
				.addOptions(NodeShape)
				.setValue(this.settings.nodeShape)
				.onChange(async (value: string) => {
					this.settings.nodeShape = value as NodeShape;
					await updateDisplayAndColors();
				}),
		);
		new Setting(container).setName("Tag shape").addDropdown((dd) =>
			dd
				.addOptions(NodeShape)
				.setValue(this.settings.tagShape)
				.onChange(async (value: string) => {
					this.settings.tagShape = value as NodeShape;
					await updateDisplayAndColors();
				}),
		);
		new Setting(container).setName("Attachment shape").addDropdown((dd) =>
			dd
				.addOptions(NodeShape)
				.setValue(this.settings.attachmentShape)
				.onChange(async (value: string) => {
					this.settings.attachmentShape = value as NodeShape;
					await updateDisplayAndColors();
				}),
		);
	}
	...
	private renderLabelSettings(container: HTMLElement) {
		...
		new Setting(container)
			...
			.setDesc(
				"If you enable this, please reopen the graph view to see the labels.",
			)
			.addToggle((toggle) =>
				toggle
					.setValue(this.settings.showNodeLabels)
					.onChange(async (value) => {
		...
		new Setting(container)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(this.settings.showLabelsOnHoverOnly)
					.onChange(async (value) => {
		...
		new Setting(container).setName("Label distance").addSlider((s) =>
			s
				.setLimits(50, 500, 10)
				.setValue(this.settings.labelDistance)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(this.settings.labelOcclusion)
					.onChange(async (value) => {
	...
	private renderInteractionSettings(container: HTMLElement) {
		...
		new Setting(container)
			.setName("Use Keyboard Controls (WASD)")
			.addToggle((toggle) =>
				toggle
					.setValue(this.settings.useKeyboardControls)
					.onChange(async (value) => {
						this.settings.useKeyboardControls = value;
						await this.plugin.saveSettings();
						this.updateControls();
					}),
			);

		new Setting(container).setName("Keyboard move speed").addSlider((s) =>
			s
				.setLimits(0.1, 10, 0.1)
				.setValue(this.settings.keyboardMoveSpeed)
				.setDynamicTooltip()
				.onChange(async (v) => {
					this.settings.keyboardMoveSpeed = v;
					await this.plugin.saveSettings();
				}),
		);

		new Setting(container).setName("Zoom on click").addToggle((toggle) =>
			toggle
				.setValue(this.settings.zoomOnClick)
				.onChange(async (value) => {
		...
		new Setting(container).setName("Rotation speed").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.rotateSpeed)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container).setName("Pan speed").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.panSpeed)
				.setDynamicTooltip()
				.onChange(async (v) => {
		...
		new Setting(container).setName("Zoom speed").addSlider((s) =>
			s
				.setLimits(0.1, 5, 0.1)
				.setValue(this.settings.zoomSpeed)
				.setDynamicTooltip()
				.onChange(async (v) => {
	...
	private renderForceSettings(container: HTMLElement) {
		...
		new Setting(container).setName("Center force").addSlider((slider) =>
			slider
				.setLimits(0, 1, 0.01)
		...
		new Setting(container).setName("Repel force").addSlider((slider) =>
			slider
				.setLimits(0, 20, 0.1)
	...
	private handleKeyboardMovement() {
		if (!this.settings.useKeyboardControls || this.pressedKeys.size === 0)
			return;
		...
		this.keyboardRight
			.crossVectors(this.keyboardDirection, camera.up)
			.normalize();

		this.keyboardMoveVector.set(0, 0, 0);

		if (this.pressedKeys.has("w"))
			this.keyboardMoveVector.add(this.keyboardDirection);
		if (this.pressedKeys.has("s"))
			this.keyboardMoveVector.sub(this.keyboardDirection);
		if (this.pressedKeys.has("a"))
			this.keyboardMoveVector.sub(this.keyboardRight);
		if (this.pressedKeys.has("d"))
			this.keyboardMoveVector.add(this.keyboardRight);

		if (this.keyboardMoveVector.lengthSq() > 0) {
			this.keyboardMoveVector.normalize().multiplyScalar(moveSpeed);
			this.keyboardNewPos
				.copy(camera.position)
				.add(this.keyboardMoveVector);
			this.keyboardNewTarget
	...
	initializeGraph() {
		this.app.workspace.onLayoutReady(async () => {
			...
			this.graph = Graph()(this.graphContainer)
				...
				.linkCurvature((link: ProcessedGraphLink) =>
					this.getLinkCurvature(link),
				)
				.onEngineTick(() => {
					const now = performance.now();
					if (
						now - this.lastLabelUpdateTime >
		...
		try {
			...
			if (oldNodes.length > 0) {
				const newNodeIds = new Set(
					hasNodes ? newData.nodes.map((n) => n.id) : [],
				);
				const nodesToRemove = oldNodes.filter(
			...
			if (hasNodes) {
				...
				const processedLinks = this.graph.graphData()
					.links as ProcessedGraphLink[];
				...
				if (this.counterEl) {
					this.counterEl.setText(
						`${newData.nodes.length} nodes · ${newData.links.length} links`,
					);
				...
				if (this.settings.useWasmPhysics && hasNodes) {
					...
					newData.nodes.forEach((n: GraphNode, idx: number) =>
					...
					this.physicsBridge
						...
						.catch((err) => {
							console.error(
								"[3D Graph] WASM Physics engine failed to start, falling back to D3 force engine:",
								err,
							);
						});
			...
			console.error(
				"3D Graph: An error occurred during updateData:",
				error,
			);
		} finally {
	...
	public updateColors() {
		...
		const linkHighlightColor = this.settings.useThemeColors
			? this.getCssColor(
					"--graph-node-focused",
					this.settings.colorHighlight,
				)
			: this.settings.colorHighlight;
		const linkColor = this.settings.useThemeColors
			? this.getCssColor("--graph-line", this.settings.colorLink)
			: this.settings.colorLink;
		this.graph.linkColor((link: GraphLink) =>
			this.highlightedLinks.has(link) ? linkHighlightColor : linkColor,
		);
	}
	...
	private getCssColor(variable: string, fallback: string): string {
		...
		try {
			...
			console.error(
				`3D Graph: Could not parse CSS color variable ${variable}`,
				e,
			);
			this.colorCache.set(variable, fallback);
			return fallback;
	...
	private updatePreprocessedGroups() {
		...
		this.preprocessedGroups = this.settings.groups
			.map((group) => {
	...
	private getNodeColor(node: GraphNode): string {
		...
		if (this.highlightedNodes.has(node.id)) {
			return useThemeColors
				? this.getCssColor("--graph-node-focused", colorHighlight)
				: colorHighlight;
		}
		...
		for (const group of this.preprocessedGroups) {
			...
			if (type === "path") {
				if (
					...
					node.id.toLowerCase().startsWith(group.pathQuery!)
				) {
					return group.color;
				}
			} else if (type === "tag") {
				...
				if (
					node.type === NodeType.Tag &&
					node.name.toLowerCase() === `#${tagQuery}`
				) {
					return group.color;
				}
				if (
					node.type === NodeType.File &&
					node.tags?.some((tag) => tag.toLowerCase() === tagQuery)
				) {
					return group.color;
				}
			} else if (type === "file") {
				...
				if (node.type === NodeType.File) {
					nodeContentLower =
						this.fileContentCache.get(node.id)?.lowerCaseContent ||
						"";
				}
				if (
					node.name.toLowerCase().includes(query) ||
					nodeContentLower.includes(query)
				) {
					return group.color;
				}
			}
		...
		if (useThemeColors) {
			if (node.type === NodeType.Tag)
				return this.getCssColor("--graph-tags", colorTag);
			if (node.type === NodeType.Attachment)
				return this.getCssColor("--graph-unresolved", colorAttachment);
	...
	public updateDisplay() {
		...
		this.graph.nodeThreeObject((node: GraphNode) =>
		...
		this.graph
			.linkWidth((link: GraphLink) =>
				this.highlightedLinks.has(link)
					? this.settings.linkThickness * 2
					: this.settings.linkThickness,
			)
	...
	public updateForces() {
		...
		const { centerForce, repelForce, linkForce, useWasmPhysics } =
			this.settings;
	...
	private updateLabels() {
		if (
			!this.isGraphInitialized ||
			!this.settings.showNodeLabels ||
			this.settings.performanceMode
		)
			return;
		...
		if (this.settings.labelOcclusion && this.occludersCacheDirty) {
			this.cachedOccluders = nodes
				.map((n: GraphNode) => this.nodeMeshes.get(n))
				.filter(Boolean) as THREE.Mesh[];
			this.occludersCacheDirty = false;
		}

		const raycastCullDistanceSq =
			this.RAYCAST_CULL_DISTANCE * this.RAYCAST_CULL_DISTANCE;
		const relevantOccluders = this.settings.labelOcclusion
			? this.cachedOccluders.filter(
					(mesh) =>
						camera.position.distanceToSquared(mesh.position) <
						raycastCullDistanceSq,
				)
			: [];
			...
			if (distanceSq <= fadeStartDistSq) {
				...
				opacity =
					...
					(distance - fadeStartDistance) /
						(visibleDistance - fadeStartDistance);
			}

			if (
				opacity > 0 &&
				this.settings.labelOcclusion &&
				relevantOccluders.length > 1
			) {
				const direction = this.reusableDirection
					.subVectors(this.reusableNodePosition, camera.position)
					.normalize();
	...
	private createNodeSprite(node: GraphNode): SpriteText {
		...
		sprite.color = isDarkMode
			? this.settings.labelTextColorDark
			: this.settings.labelTextColorLight;
		sprite.backgroundColor = this.hexToRgba(
			this.settings.labelBackgroundColor,
			this.settings.labelBackgroundOpacity,
		);
	...
	private updateNodeColorsDiff() {
		...
		this.highlightedNodes.forEach((id) =>
			this.lastHighlightedNodes.add(id),
		);
	}
	...
	private matchesFilter(node: GraphNode, filter: Filter): boolean {
		...
		if (filter.type === "tag") {
			const tagToMatch = filterValue.startsWith("#")
			...
			return (
				node.tags?.some((tag) => tag.toLowerCase() === tagToMatch) ??
				false
			);
		}
		return false;
	...
	private async processVaultData(): Promise<{
		...
		const needsContent =
			!!searchQuery ||
			this.preprocessedGroups.some((g) => {
				return g.type === "text";
			});

		const CONCURRENCY_LIMIT = 50;
		const readQueue = [...allFiles];
		const workers = Array(Math.min(CONCURRENCY_LIMIT, readQueue.length))
			.fill(null)
			.map(async () => {
				while (readQueue.length > 0) {
					...
					if (
						cachedNodeInfo &&
						cachedNodeInfo.mtime === file.stat.mtime
					) {
						...
						if (
							node.type === NodeType.File &&
							needsContent &&
							!this.fileContentCache.has(file.path)
						) {
							const content =
						...
						if (type === NodeType.File && needsContent) {
							const cachedContent = this.fileContentCache.get(
								file.path,
							);
							if (
								!cachedContent ||
								cachedContent.mtime !== file.stat.mtime
							) {
								const content =
									await this.app.vault.cachedRead(file);
								const lowerCaseContent = content.toLowerCase();
						...
						this.processedNodes.set(file.path, {
							node,
							mtime: file.stat.mtime,
						});
					}
		...
		if (showTags) {
			...
			allTags.forEach((tagNode, tagName) =>
				allNodesMap.set(tagNode.id, tagNode),
			);
		}
		...
		if (searchQuery) {
			...
			matchedNodes = matchedNodes.filter((node) => {
				...
				return (
					...
					(node.type !== NodeType.Tag &&
						node.id.toLowerCase().includes(lowerCaseFilter)) ||
		...
		if (showNeighboringNodes && (activeFilters.length > 0 || searchQuery)) {
			...
			finalNodes = Array.from(allNodesMap.values()).filter(
				(node) => matchedIds.has(node.id) || neighborIds.has(node.id),
			);
		} else {
		...
		const linksToShow = allLinks.filter(
			(link) =>
				nodesToShowIds.has(link.source) &&
				nodesToShowIds.has(link.target),
		);
	...
	private cleanupNode(
		node: GraphNode,
		options: { cleanMesh?: boolean; cleanGroup?: boolean } = {
			cleanMesh: true,
			cleanGroup: true,
		},
	) {
	...
	async onClose() {
		...
		if (this.graph) {
			this.graph
				.graphData()
				.nodes.forEach((node: GraphNode) => this.cleanupNode(node));
